...
import { GET_UNDER_THOUSAND_PRODUCTS } from "../constants/actionTypes";
...
export default function UnderThousand() {
  ...
  useEffect(() => {
    let CategoryList = [];
    const { products } = underThousandProducts;
    for (const { categoryID, categoryName, categorySlug } of products) {
      ...
      if (CategoryList.length < 1 || indexof === -1) {
        CategoryList.push({
          ...
          slug: categorySlug,
          count: 1,
          url: `/under999?category=${categorySlug}`,
  ...
  }, [underThousandProducts, catSlug, subcatSlug, subsubcatSlug]);
      ...
      if (catID === categoryID) {
        if (childrens.length < 1 || indexof === -1) {
          childrens.push({
            ...
            count: 1,
            url: `/under999?category=${categorySlug}&subcategory=${subCategorySlug}`,
      ...
      if (catID === categoryID && subcatID === subCategoryID) {
        if (childrens.length < 1 || indexof === -1) {
          childrens.push({
            ...
            count: 1,
            url: `/under999?category=${categorySlug}&subcategory=${subCategorySlug}&subsubcategory=${subSubCategorySlug}`,
            className: `${
  ...
  const filter = () => {
    if (subsubcatSlug) {
      return {
        ...underThousandProducts,
        products: underThousandProducts.products.filter(
    ...
    <>
      ...
      <ProductList
        ...
        MobileListing={{
          ...
          sublist: SubList,
          modalTitle: { text: "All Underthousands", url: `` },
        }}
      >
